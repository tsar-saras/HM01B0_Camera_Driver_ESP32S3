...
// {
//     // Completely bypass sync waits to stop the ESP32 from hanging
//     for (uint32_t i = 0; i < ui32BufferLen; i++)
//     {
//         *(pui8Buffer + i) = HM01B0_READ_BYTE;
//     }
...
//     // --- ADD THIS BLOCK ---
//     // Wait for VSYNC to go HIGH then LOW to ensure we start at the top of a new frame
//     while (0x00 == HM01B0_READ_VSYNC); 
//     while (HM01B0_READ_VSYNC); 
//     // ----------------------
...
//     return HM01B0_ERR_OK;
// }
hm01b0_status_e IRAM_ATTR hm01b0_blocking_read_oneframe(hm01b0_cfg_t *psCfg, uint8_t *pui8Buffer, uint32_t ui32BufferLen)
{
    register uint8_t *ptr = pui8Buffer;
    
    // 1. BULLETPROOF VSYNC DEBOUNCE
    // Force VSYNC to stay LOW for 10,000 loops to clear all electrical noise
    uint32_t low_count = 0;
    while (low_count < 10000) {
        if ((REG_READ(GPIO_IN_REG) & (1 << 3)) == 0) {
            low_count++;
        } else {
            low_count = 0;
        }
    ...
    // Now wait for the exact, clean start of the new frame (VSYNC HIGH)
    while (!(REG_READ(GPIO_IN_REG) & (1 << 3)));

    // 2. THE TRUE 324x244 GRID
    for (uint32_t row = 0; row < 244; row++) 
    {
        while (!(REG_READ(GPIO_IN_REG) & (1 << 5))); // Wait HSYNC HIGH

        for (uint32_t col = 0; col < 324; col++)
        {
            register uint32_t val;
            while (!((val = dedic_gpio_bundle_read_in(rx_bundle)) & 0x01)); // Wait PCLK HIGH
            *ptr++ = (uint8_t)val;
            while (dedic_gpio_bundle_read_in(rx_bundle) & 0x01); // Wait PCLK LOW
        }
        
        while (REG_READ(GPIO_IN_REG) & (1 << 5)); // Wait HSYNC LOW
    }
